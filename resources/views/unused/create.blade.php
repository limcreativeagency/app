...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if(form && phoneInput && phoneFull && window.intlTelInput) {
        ...
        // Sayfa yüklendiğinde mevcut numarayı işle
        if (phoneInput.value) {
            let currentNumber = phoneInput.value;
            if (iti.getSelectedCountryData().iso2 === 'tr') {
                currentNumber = currentNumber.replace(/\s+/g, '');
                if (!currentNumber.startsWith('+90')) {
                    currentNumber = '+90' + currentNumber;
                }
                phoneFull.value = currentNumber;
            }
        }

        // Form gönderilmeden önce numarayı işle
        form.addEventListener('submit', function() {
            if(iti && typeof iti.getNumber === 'function') {
                let fullNumber = iti.getNumber();
                
                if (iti.getSelectedCountryData().iso2 === 'tr') {
                    // Türkiye numarası için özel işlem
                    fullNumber = fullNumber.replace(/\s+/g, '');
                    if (!fullNumber.startsWith('+90')) {
                        fullNumber = '+90' + fullNumber;
                    }
                }
        ...
        // Telefon numarası değiştiğinde işle
